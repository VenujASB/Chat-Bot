...
async def mizuki(client, message):
    ...
    try:
        L = await fetch(f"https://api.affiliateplus.xyz/api/chatbot?message={finaltxt}&botname=AbhishekBot&ownername=Abhishek&user=1")
        msg = L["message"]        
...
async def start(client, message):
   if message.chat.type == 'private':
       await message.reply("**Hey There, I'm Abhishek. An advanced chatbot with AI. \n\nAdd me to your group and chat with me!**",   
                            reply_markup=InlineKeyboardMarkup(
                                [[
                                        InlineKeyboardButton(
                                            "Dev", url="https://t.me/VenujaST"),
                                        InlineKeyboardButton(
                                            "Repo", url="https://github.com/VenujASB/Chat-Bot")
                                    ]]
                            ),               
           )
   else:
